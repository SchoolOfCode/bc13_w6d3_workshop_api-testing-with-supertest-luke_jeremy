...
Remember: if you get stuck, look at the docs for both Jest and Supertest.

1. `cd` into `task-1` folder. Run `npm i` to install required dependencies.
2. Familiarise yourself with the contents and structure of the `task-1` folder. You haven't written it, but there also shouldn't be anything new or unfamiliar in there.
...
2. Set up a brand new PostgreSQL database (using a dbaas/cloud service or locally). (It should be new as we will be repeatedly creating and dropping a `users` table.)
